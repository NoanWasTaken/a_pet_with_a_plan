                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('commandes.update', $commande) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="id_utilisateur" id="id_utilisateur" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                ...
                                @foreach($utilisateurs as $utilisateur)
                                    <option value="{{ $utilisateur->id }}" {{ old('id_utilisateur', $commande->id_utilisateur) == $utilisateur->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="date" name="date_commande" id="date_commande" value="{{ old('date_commande', \Carbon\Carbon::parse($commande->date_commande)->format('Y-m-d')) }}" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                            @error('date_commande')
                                <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                                    ...
                                    <div class="produit-item border dark:border-gray-600 p-4 rounded mb-3">
                                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            <div>
                                                ...
                                                <select name="produits[{{ $index }}][id]" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                                    <option value="">Choisir un produit</option>
                                                    @foreach($produits as $p)
                                                        <option value="{{ $p->id }}" {{ $produit->id == $p->id ? 'selected' : '' }}>
                                                            {{ $p->nom }} ({{ number_format($p->prix_euros, 2) }}€)
                                                        </option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div>
                                                ...
                                                <input type="number" name="produits[{{ $index }}][quantite]" value="{{ old('produits['.$index.'][quantite]', $produit->pivot->quantite) }}" min="1" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                            </div>
                                            <div class="flex items-end">
                                                <button type="button" class="remove-produit bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-3 rounded text-sm">
                                                    Retirer
                                                </button>
                                            </div>
                                        </div>
                        ...
                        <div class="flex items-center justify-end mt-6">
                            <a href="{{ route('commandes.show', $commande) }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded mr-2">
                                Annuler
                            </a>
                    ...
                    <script>
                        document.addEventListener('DOMContentLoaded', function() {
                            ...
                            let produitIndex = {{ count($commande->produits) > 0 ? count($commande->produits) - 1 : 0 }};
                            ...
                            addButton.addEventListener('click', function() {
                                ...
                                selects.forEach(select => {
                                    select.name = select.name.replace(/\[\d+\]/, `[${produitIndex}]`);
                                    select.value = '';
                                });
                                
                                inputs.forEach(input => {
                                    input.name = input.name.replace(/\[\d+\]/, `[${produitIndex}]`);
                                    input.value = '1';
                                });
